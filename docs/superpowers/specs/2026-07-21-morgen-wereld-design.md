...
2. **Wereld: campus-dorp** in een tuinachtige setting. Vier gebouwen rond een Morgen-plein met de geel-groene punt als landmark: collegezaal (Train), werkplaats (Implement), lab (Build), theater (Inspire). Zones vloeien visueel in elkaar over; Train en Implement zijn het sterkst verweven (gedeeld terrein, doorlopend pad). Daarnaast drie kleinere elementen (besluit 2026-07-21): een **houten wegwijzer** bij de bol (klik opent het Kompas), een **maquette-tuin** langs het pad met gerealiseerde projecten als mini-maquettes op sokkels (klik opent projecten; groeit mee per project), en een **kampvuur-kring** in een rustige tuinhoek voor over ons/het team. De **brug aan de voorrand verbindt het eiland zichtbaar met de buitenwereld**: het pad loopt door tot buiten het kader (de aankomstroute van de intro-vlucht), zodat de campus geen geïsoleerd eiland is maar een plek waar je wordt binnengelaten. In de PoC zijn deze drie elementen alleen visueel aanwezig in de hub-still; klikgedrag volgt in de uitbouwfase.
